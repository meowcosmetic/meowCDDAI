...
class ObjectDetector:
    ...
    def detect(self, frame: np.ndarray, prioritize_book: bool = True, frame_count: int = 0) -> List[Dict[str, Any]]:
        ...
        """
        # ✅ Chỉ dùng OID detector (COCO đã bị xóa)
        if self.oid_detector and self.oid_detector.is_available:
            try:
        ...
        # Không có OID detector - không thể detect objects
        logger.warning("[ObjectDetector] OID detector không available, không thể detect objects")
        return []
